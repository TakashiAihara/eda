...
- **map** vs **map.md**: the *map* is the tree held in `eda.json` by the server. `map.md` is an export of it and is never read back as truth.
...
| pending slot | At most one AI suggestion per (session, model) waits at a time | `suggest()` |
