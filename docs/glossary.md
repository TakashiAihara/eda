...
| marker | One of a fixed set on a node: priority-1..3, doing, done, flag, star, question. A priority and a progress state each exclude the others of their group. Set by a person only; the AI reads them | `Node.markers`, `MARKERS` / `toggleMarker` in `src/map.ts` |
