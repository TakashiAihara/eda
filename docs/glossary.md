...
| main topic | A child of the drawn root (XMind's term); it and its subtree share one colour, picked from its id | `topicColour` in `web/view.ts`, `--b0`..`--b5` in `web/style.css` |
