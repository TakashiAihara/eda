...
- The level limit (Alt+1..9, Alt+0 for all) hides what is below that level under the drawn root without touching `collapsed`, so it is not saved and a reload shows every level. The header shows the limit with a way back, and the fold button at the cut shows every level again rather than saving a fold (a node already collapsed there keeps its own fold button). Suggestions below the limit are not drawn on the map; the sidebar lists them. Adding a child to a node at the limit shows one more level, so the new node is not saved out of sight. With Alt, digit keys are read from their position (`KeyboardEvent.code`), since Option+1 on a Mac types ¡. At the limit, `+` shows one more level and `-` does nothing (there is no fold on screen to save). The header shows the limit only while it hides something. Known limit: Chrome and Firefox on Linux keep Alt+1..8 for switching tabs, which the page may not be able to take (not checked on a Linux desktop; macOS and Windows use other keys for tabs).
