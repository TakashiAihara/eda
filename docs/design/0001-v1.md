...
- Known limit: Claude Code has a shell and can read `~/.eda/token`, so it could call the HTTP API directly. The tools and instructions do not offer that path; closing it fully would need the browser to hold a credential the agent's user cannot read, which a single-user host cannot provide.
...
| several models suggesting in parallel | later; the pending slot is already per model |
